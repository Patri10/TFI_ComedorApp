  ...
  async createPurchase(purchase: Purchase): Promise<Purchase> {
    ...
    if (purchaseError) {
      throw new HttpException(
        'Compra no creada: ' + purchaseError.message,
    ...
    if (existError || !exists) {
      throw new HttpException(
        'Error de integridad: La compra cabecera no se encuentra para asociar detalles.',
    ...
    if (
      ...
      purchase.getPurchaseDetails().length > 0
    ) {
      ...
      if (detailsError) {
        ...
        throw new HttpException(
          'Error al crear los detalles de la compra: ' + detailsError.message,
          HttpStatus.BAD_REQUEST,
        );
      }
    }

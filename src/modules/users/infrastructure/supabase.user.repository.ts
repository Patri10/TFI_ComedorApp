...
export class SupabaseUserRepository implements UserRepository {
  ...
  async createUser(user: User): Promise<any> {
    ...
    if (error) {
      throw new HttpException(
        'Usuario no creado: ' + error.message,
  ...
  private async deleteAuthUser(user: User, id: string) {
    const { data, error } =
      await this.supabaseAdminClient.auth.admin.deleteUser(id);

    if (error) {
      console.log(
        'Usuario no eliminado: no se pudo hacer rollbakc del usuario',
      );
      throw new HttpException(
        'Usuario no eliminado: ' + error.message,
        HttpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  async updateUser(id: string, user: User): Promise<void> {
    ...
    if (Object.keys(updates).length === 0) {
      const { error: authError } =
        await this.supabaseAdminClient.auth.admin.updateUserById(id, updates);
      if (authError) {
